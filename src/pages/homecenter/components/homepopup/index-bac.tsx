import { useNavigate } from "react-router-dom";
import { Popup, Space, Button, Toast } from "antd-mobile";
import copy from "copy-to-clipboard";
import "./index.css";
import { useTranslation } from "react-i18next";
import { changeThem, getText, isDark } from "../../../../utils/util";
import { useState } from "react";
import Search from "../../../../components/search";
import { imageConfig } from "../../../../config/config";
import { Badge } from "antd";

export default function HomePopup({
  isShowHomePop,
  setIShowHomePop,
  userInfo,
  loginmsg
}) {
  const lan = localStorage.getItem("i18n");
  const navigate = useNavigate();
  const { t: translate } = useTranslation();
  const types = ["未認證", "審核中", "已認證", "審核拒絕"];
  const handleCopy = (value) => {
    if (copy(value)) {
      Toast.show({ content: translate(getText("成功")) });
    }
  };
  return (
    <Popup
      visible={isShowHomePop}
      onMaskClick={() => {
        setIShowHomePop(false);
      }}
      position="left"
      bodyStyle={{ backgroundColor: "var(--them-background)", width: "75vw" }}
    >
      <div className="homePopCenter-1">
        <div className="homePopCenter-2">
          <div className="homePopCenter-3">
            {/* {isDark() && (
              <div
                className="homePopCenter-4-dark"
                onClick={() => {
                  changeThem("light");
                }}
              ></div>
            )}
            {!isDark() && (
              <div
                className="homePopCenter-4-light"
                onClick={() => {
                  changeThem("dark");
                }}
              ></div>
            )} */}
          </div>
        </div>
        <div className="homePopCenter-11">
          <div className="homePopCenter-12">
            <div className="homePopCenter-13"></div>
            <span className="homePopCenter-14"></span>
            <img
              src=""
              draggable="false"
              className="homePopCenter-15"
            />
          </div>
          <div className="homePopCenter-16">
            <div className="homePopCenter-17">
              <div className="homePopCenter-18"></div>
              <div className="homePopCenter-19">
                <div className="homePopCenter-20">
                  <div className="homePopCenter-21"></div>
                </div>
                <div className="homePopCenter-22">
                  <div className="homePopCenter-23"></div>
                </div>
              </div>
              <img
                src=""
                draggable="false"
                className="homePopCenter-24"
              />
            </div>
            <div className="homePopCenter-25">
              <span className="homePopCenter-26">VIP1</span>
            </div>
          </div>
        </div>
        <div
          className="homePopCenter-27"
          style={{
            color: isDark() ? "#fff" : "",
          }}
        >
          <span className="homePopCenter-28">{userInfo?.username}</span>
        </div>
        <div
          className="homePopCenter-29"
          onClick={() => {
            handleCopy(userInfo?.userCode);
          }}
        >
          <div className="homePopCenter-30">
            <span className="homePopCenter-31">{translate(getText("ID"))}：{userInfo?.userCode}</span>
          </div>
          <div className="homePopCenter-32">
            <img
              src=""
              draggable="false"
              className="homePopCenter-35"
            />
          </div>
        </div>
        <div
          className="homePopCenter-36"
          onClick={() => {
            if (userInfo?.rzstatus === 2) {
              return;
            }
            navigate("/idcard");
          }}
        >
          <div className="homePopCenter-37">{translate(getText("身份認證"))}</div>
          <div className="homePopCenter-38">
            <span className="homePopCenter-39">
              {translate(getText(types[userInfo?.rzstatus]))}
            </span>
          </div>
          <div className="homePopCenter-40">
            <div className="homePopCenter-41"></div>
            <span className="homePopCenter-42"></span>
            <img
              src=""
              draggable="false"
              className="homePopCenter-43"
            />
          </div>
        </div>
        {userInfo?.rzstatus == 2 && (
          <div
            className="homePopCenter-44"
            onClick={() => {
              if (userInfo?.rzstatus === 2 && userInfo?.cardsc) {
                return;
              }
              navigate("/gjidcard");
            }}
          >
            <div className="homePopCenter-45">{translate(getText("高级認證"))}</div>
            <div className="homePopCenter-46">
              <span className="homePopCenter-47">
                {userInfo?.rzstatus === 2 &&
                  userInfo?.cardsc &&
                  translate(getText(types[userInfo?.rzstatus]))}
                {!(userInfo?.rzstatus === 2 && userInfo?.cardsc) &&
                  translate(getText(types[0]))}
              </span>
            </div>
            <div className="homePopCenter-48">
              <div className="homePopCenter-49"></div>
              <span className="homePopCenter-50"></span>
              <img
                src="data:image/png;base64,"
                draggable="false"
                className="homePopCenter-51"
              />
            </div>
          </div>
        )}
        <div
          className="homePopCenter-52"
          style={{
            color: isDark() ? "" : "rgb(51, 51, 51)",
          }}
        >
          <div
            className="homePopCenter-53"
            onClick={() => {
              navigate("/jyjl");
            }}
          >
            <div className="homePopCenter-54">
              <span className="homePopCenter-55">
                {translate(getText("交易記錄"))}
              </span>
            </div>
          </div>
          <div
            className="homePopCenter-56"
            onClick={() => {
              navigate("/sharecenter");
            }}
          >
            <div className="homePopCenter-57">
              <span className="homePopCenter-58">{translate(getText("分享"))}</span>
            </div>
          </div>
          {/* <div
            className="homePopCenter-56"
            onClick={() => {
              navigate("/myteam");
            }}
          >
            <div className="homePopCenter-57">
              <span className="homePopCenter-58">{translate(getText("我的团队"))}</span>
            </div>
          </div> */}
          <div
            className="homePopCenter-59"
            onClick={() => {
              navigate("/noice");
            }}
          >
            <div className="homePopCenter-60">
              <span className="homePopCenter-61">
              {translate(getText("留言列表"))}
              <Badge count={loginmsg?.noticeCount ? loginmsg?.noticeCount : 0} offset={[7, -15]}></Badge>
              </span>
            </div>
          </div>
          
          <div
            className="homePopCenter-59"
            onClick={() => {
              navigate("/helplist");
            }}
          >
            <div className="homePopCenter-60">
              <span className="homePopCenter-61">
                {translate(getText("幫助中心"))}
              </span>
            </div>
          </div>
          <div
            className="homePopCenter-62"
            onClick={() => {
              navigate("/aboutus");
            }}
          >
            <div className="homePopCenter-63">
              <span className="homePopCenter-64">{translate(getText("平台介绍"))}</span>
            </div>
          </div>
          <div
            className="homePopCenter-65"
            onClick={() => {
              navigate("/setting");
            }}
          >
            <div className="homePopCenter-66">
              <span className="homePopCenter-67">{translate(getText("设置"))}</span>
            </div>
          </div>
          <div
            className="homePopCenter-65"
            onClick={() => {
              navigate("/download");
            }}
          >
            <div className="homePopCenter-66">
              <span className="homePopCenter-67">
                {" "}
                App{(lan == "zh" ? "" : " ") + translate(getText("下載"))}
              </span>
            </div>
          </div>
          <div
            className="homePopCenter-68"
            onClick={() => {
              navigate("/securitycenter");
            }}
          >
            <div className="homePopCenter-69">
              <span className="homePopCenter-70">
                {translate(getText("安全中心"))}
              </span>
            </div>
          </div>
        </div>
      </div>
    </Popup>
  );
}
