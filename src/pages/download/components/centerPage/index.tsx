import { useTranslation } from "react-i18next";
import { useNavigate } from "react-router-dom";
import { Button } from "antd-mobile";
import { getText } from "../../../../utils/util";
import "./index.css";
import { imageConfig } from "../../../../config/config";

export default function DownlandPage({ companyData }) {
  const navigate = useNavigate();
  const { t: translate } = useTranslation();
  const title = localStorage.getItem("title");
  return (
    <div className="downloadCenter-1">
      <div className="downloadCenter-2">
        <div className="downloadCenter-3"></div>
        <div className="downloadCenter-4">
          <div className="downloadCenter-5">
            <div className="downloadCenter-6"></div>
          </div>
          <div className="downloadCenter-7">
            <div className="downloadCenter-8"></div>
          </div>
        </div>
        <img
          src="/assets/downloadBg-fe9eece8.png"
          draggable="false"
          className="downloadCenter-9"
        />
      </div>
      <div className="downloadCenter-10">
        <img
          src={imageConfig.baseImageUrl + companyData?.companyLogo}
          draggable="false"
          className="downloadCenter-13"
        />
      </div>
      <div className="downloadCenter-14">
        <span className="downloadCenter-15">{companyData?.companyName}</span>
      </div>
      <div
        className="downloadCenter-16"
        onClick={() => {
          if (companyData?.iosDomain.includes("http")) {
            window.location.href = companyData?.iosDomain;
          }
        }}
      >
        <div className="downloadCenter-17">
          <div className="downloadCenter-18"></div>
          <div className="downloadCenter-19">
            <div className="downloadCenter-20">
              <div className="downloadCenter-21"></div>
            </div>
            <div className="downloadCenter-22">
              <div className="downloadCenter-23"></div>
            </div>
          </div>
          <img
            src="data:image/png;base64,"
            draggable="false"
            className="downloadCenter-24"
          />
        </div>
        iOS{translate(getText("下載"))}
      </div>
      <div
        className="downloadCenter-25"
        onClick={() => {
          if (companyData?.androidDomain.includes("http")) {
            window.location.href = companyData?.androidDomain;
          }
        }}
      >
        <div className="downloadCenter-26">
          <div className="downloadCenter-27"></div>
          <div className="downloadCenter-28">
            <div className="downloadCenter-29">
              <div className="downloadCenter-30"></div>
            </div>
            <div className="downloadCenter-31">
              <div className="downloadCenter-32"></div>
            </div>
          </div>
          <img
            src="data:image/png;base64,"
            draggable="false"
            className="downloadCenter-33"
          />
        </div>
        Android{translate(getText("下載"))}
      </div>
      <div className="downloadCenter-34">
        <span
          className="downloadCenter-35"
          onClick={() => {
            navigate("/downloadbook");
          }}
        >
          {translate(getText("安装说明"))}
        </span>
      </div>
    </div>
  );
}
